...
def _score_image(
    ...
    for key in (
        ...
        if inter:
            ...
            # AI 카테고리와 다른 카테고리 매칭 시 가중치 감소
            if key == "category":
                # AI 카테고리와 완전히 다른 카테고리면 점수 감소
...
def final_recommendation(
    ...
    try:
        ...
        # 가중치 조정 - AI 분석 카테고리 우선, 사용자 요청 반영
        categories = weighted_union(
            (
                ai_attributes.get("category", []) if ai_attributes else [],
                10,
            ),  # AI 카테고리 최우선
            (user_style_prefs.get("style", []), 2),  # 사용자 스타일
            (prompt_info.get("keywords", []), 1),
            (weather_attrs.get("category", []), 0.5),  # 날씨 영향 최소
        )
        ...
        colors = weighted_union(
            ...
            (user_style_prefs.get("color", []), 3 * style_weight),  # 색상 선호도
        ...
        details = weighted_union(
            ...
            (user_style_prefs.get("detail", []), 2.5 * style_weight),  # 디테일
...
"""
        ...
        try:
            ...
            logger.warning("LLM 호출 타임아웃. 폴백 메시지 사용.")
